...
async function getStatuses(_: Request, res: Response) {
    try {
...
async function deleteStatus(req: Request, res: Response) {
    try {
        ...
        res.status(500).json({ message: "Internal server error" });
        console.log(`Error in deleteStatus for status of id: ${req.params.id}`, error);
    }
}
...
async function updateStatus(req: Request, res: Response) {
    try {
        ...
        console.log(`Error in updateStatus for status of id: ${req.params.id}`, error);
    }
}

export default { getStatuses, getStatus, createStatus, deleteStatus, updateStatus }