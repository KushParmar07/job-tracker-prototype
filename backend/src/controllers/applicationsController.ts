...
async function getApplication(req: Request, res: Response) {
    try {
        ...
        res.status(500).json({message: "Internal server error"})
        console.log(`Error in getApplication for application of id: ${req.params.id}`, error)
    }
}
...
async function deleteApplication(req: Request, res: Response) {
    try {
        ...
        res.status(500).json({message: "Internal server error"})
        console.log(`Error in deleteApplication for application of id: ${req.params.id}`, error)
    }
}
...
export default { getApplications, getApplication, createApplication, deleteApplication, updateApplication };
